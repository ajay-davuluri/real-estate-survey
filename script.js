// ============================================================
// PROPERTY SEARCH SURVEY — Semantic ID Architecture
// ============================================================
// Key principles:
// 1. Each question has a SEMANTIC id (not positional)
// 2. Position determined ONLY by array order
// 3. Add/remove/reorder by editing the questions array
// 4. Transitions, skip logic, exit logic defined INSIDE each question
// 5. Total count auto-derived from array length
// 6. Google Sheet submission sends ALL answer keys dynamically
// ============================================================

const answers = {};
let currentQuestion = 0; // 0 = welcome

const questions = [
    // ─── WELCOME ───
    { id: 'welcome', type: 'welcome', section: null },

    // ─── SECTION 2: Your Current Property Search Context ───
    {
        id: 'currently_searching',
        type: 'single',
        section: 'Section 2: Your Current Property Search Context',
        title: 'Are you currently searching for a property?',
        options: ['Yes, actively searching', 'Planning to start within 3 months', 'Planning to start within 12 months', 'Not currently searching'],
        transition: () => '👋 Let\'s start with where you are in your property journey...',
        exitIf: (answers) => answers.currently_searching === 'Not currently searching',
        exitMessage: '🙏 Thank you! Your feedback still helps us understand the market.'
    },
    {
        id: 'property_type',
        type: 'multiple',
        section: 'Section 2: Your Current Property Search Context',
        title: 'What type of property are you looking for?',
        hint: 'Select all that apply',
        options: ['Apartment', 'Villa', 'Independent House', 'Plot', 'Commercial Property', 'Investment Property', 'Other'],
        transition: (answers) => {
            if (answers.currently_searching === 'Yes, actively searching') return '🔥 You\'re in the thick of it! Let\'s understand what you\'re looking for.';
            if (answers.currently_searching === 'Planning to start within 3 months') return '📋 Smart to plan ahead! Let\'s map out your preferences.';
            if (answers.currently_searching === 'Planning to start within 12 months') return '🌱 Great timing to start early! Let\'s explore your preferences.';
            return '💡 Your past experience is valuable! Tell us what you were looking for.';
        }
    },
    {
        id: 'primary_goal',
        type: 'single',
        section: 'Section 2: Your Current Property Search Context',
        title: 'What is the primary purpose of your property search?',
        options: ['Primary Residence', 'Investment', 'Rental Income', 'Vacation Home', 'Business Use', 'Other'],
        transition: (answers) => {
            if (answers.property_type && answers.property_type.length > 2) return '🏘️ Quite a diverse search! Now tell us the main purpose...';
            if (answers.property_type && answers.property_type.includes('Villa')) return '🏡 Villa living — exciting! What\'s the main goal for this property?';
            if (answers.property_type && answers.property_type.includes('Plot')) return '📐 Land hunting — the foundation of something big! What\'s your goal?';
            return '👍 Got it! Now, what\'s the primary purpose of this property?';
        }
    },
    {
        id: 'locations',
        type: 'text',
        section: 'Section 2: Your Current Property Search Context',
        title: 'Which city/area are you searching in?',
        placeholder: 'e.g., Hyderabad, Bangalore, Mumbai...',
        transition: (answers) => {
            if (answers.primary_goal === 'Investment') return '📈 Smart investor mindset! Where are you looking to invest?';
            if (answers.primary_goal === 'Primary Residence') return '🏠 Finding home sweet home! Where are you searching?';
            if (answers.primary_goal === 'Rental Income') return '💰 Building passive income — great strategy! Which areas?';
            return '✅ Understood! Now, which locations are on your radar?';
        }
    },
    {
        id: 'budget_range',
        type: 'single',
        section: 'Section 2: Your Current Property Search Context',
        title: 'What is your budget range?',
        options: ['Under ₹25 Lakhs', '₹25-50 Lakhs', '₹50 Lakhs - 1 Crore', '₹1-2 Crore', '₹2-5 Crore', '₹5-10 Crore', '₹10+ Crore'],
        transition: (answers) => {
            if (answers.locations && answers.locations.trim()) return `📍 ${answers.locations.split(',')[0].trim()} — nice area! Now let\'s talk budget...`;
            return '📍 Location noted! Let\'s talk about your budget range...';
        }
    },
    {
        id: 'confidence_level',
        type: 'rating',
        section: 'Section 2: Your Current Property Search Context',
        title: 'How confident are you in finding the right property within your budget?',
        labelLow: 'Not confident',
        labelHigh: 'Very confident',
        transition: (answers) => {
            if (answers.budget_range && answers.budget_range.includes('Crore')) return '💎 Premium segment — exciting! How confident are you about finding the right one?';
            return '💰 Budget range noted! How confident do you feel about your search?';
        }
    },
    {
        id: 'top_concern',
        type: 'text',
        section: 'Section 2: Your Current Property Search Context',
        title: 'What concerns you most right now about finding a property?\n\n(For example: pricing transparency, hidden charges, legal issues, builder credibility, location safety, resale value, loan approvals, construction quality, or anything else.)',
        placeholder: 'e.g., Hidden charges, builder reputation, legal clearances, loan approval delays, property quality...',
        transition: (answers) => {
            if (answers.confidence_level >= 8) return '💪 High confidence — that\'s great! Still, what\'s your biggest worry?';
            if (answers.confidence_level >= 5) return '🤞 Moderately confident — totally normal. What concerns you most?';
            return '😟 We hear you — property search can feel overwhelming. What worries you most?';
        }
    },

    // ─── SECTION 3: Recent Search Experience ───
    {
        id: 'properties_visited',
        type: 'numeric',
        section: 'Section 3: Recent Search Experience',
        title: 'How many properties have you physically visited so far?',
        placeholder: 'Number of properties visited',
        transition: () => '🏠 Thanks for sharing! Now let\'s talk about your actual search experience...'
    },
    {
        id: 'search_duration',
        type: 'single',
        section: 'Section 3: Recent Search Experience',
        title: 'How long have you been actively searching?',
        options: ['Less than 2 weeks', '2-4 weeks', '1-3 months', '3-6 months', 'More than 6 months'],
        transition: (answers) => {
            if (answers.properties_visited >= 10) return '🏃 Wow, ' + answers.properties_visited + ' visits! That\'s a lot of legwork. How long has this taken?';
            if (answers.properties_visited >= 5) return '👟 ' + answers.properties_visited + ' visits — quite a few! How long has the search been going?';
            return '📝 Noted! How long has your search process taken so far?';
        }
    },
    {
        id: 'brokers_contacted',
        type: 'numeric',
        section: 'Section 3: Recent Search Experience',
        title: 'How many brokers/builders have you spoken with?',
        placeholder: 'Number of brokers/builders',
        transition: (answers) => {
            if (answers.search_duration === 'More than 6 months') return '⏳ Over 6 months — that\'s a marathon! How many agents have been involved?';
            if (answers.search_duration === '3-6 months') return '📅 Several months of searching. How many brokers have you dealt with?';
            return '⏰ Got it! How many brokers or agents have you interacted with?';
        }
    },
    {
        id: 'properties_shortlisted',
        type: 'numeric',
        section: 'Section 3: Recent Search Experience',
        title: 'How many properties did you shortlist?',
        placeholder: 'Number of shortlisted properties',
        transition: (answers) => {
            if (answers.brokers_contacted >= 5) return '🤝 ' + answers.brokers_contacted + ' agents — that\'s a lot of conversations! How many properties made your shortlist?';
            return '📋 Noted! Out of all those properties, how many made the shortlist?';
        }
    },
    {
        id: 'avoidable_visits',
        type: 'numeric',
        section: 'Section 3: Recent Search Experience',
        title: 'How many property visits felt avoidable or wasted?',
        placeholder: 'Number of avoidable visits',
        transition: (answers) => {
            if (answers.properties_shortlisted <= 2) return '🎯 Only ' + answers.properties_shortlisted + ' shortlisted — finding the right one is tough! How many visits felt wasted?';
            return '📝 ' + answers.properties_shortlisted + ' shortlisted. Now honestly — how many visits could have been avoided?';
        }
    },
    {
        id: 'stress_level',
        type: 'rating',
        section: 'Section 3: Recent Search Experience',
        title: 'How stressful has the property search process been?',
        labelLow: 'Not stressful',
        labelHigh: 'Extremely stressful',
        transition: (answers) => {
            if (answers.avoidable_visits >= 5) return '😤 ' + answers.avoidable_visits + ' avoidable visits — that\'s frustrating time wasted. How stressful was it overall?';
            if (answers.avoidable_visits >= 2) return '🤔 A few wasted trips. How stressful was the whole process?';
            return '👍 Not too many wasted visits! Still, how stressful was the overall search?';
        }
    },
    {
        id: 'time_consuming_part',
        type: 'text',
        section: 'Section 3: Recent Search Experience',
        title: 'Which part of the property search process consumed the most time?\n\n(Parts of property search include: browsing listings online, shortlisting properties, scheduling visits, physically visiting properties, negotiating price, verifying legal documents, arranging financing/loans, coordinating with brokers, or comparing options.)',
        placeholder: 'e.g., Visiting too many properties, verifying legal documents, negotiating with sellers, getting loan approvals...',
        transition: (answers) => {
            if (answers.stress_level >= 8) return '😓 That sounds really stressful. Which part consumed the most time?';
            if (answers.stress_level >= 5) return '😐 Moderate stress — understandable. Which part ate up the most time?';
            return '😊 Relatively low stress — nice! Which part of the process took the most time?';
        }
    },
    {
        id: 'one_thing_to_change',
        type: 'text',
        section: 'Section 3: Recent Search Experience',
        title: 'If you could change one thing about your property search experience, what would it be?\n\n(Think about: how you discover properties, how brokers communicate, how visits are scheduled, how pricing is shared, how legal verification works, how loans are processed, or how decisions are made.)',
        placeholder: 'e.g., Better filtering before visits, more transparent pricing, fewer irrelevant recommendations...',
        transition: () => '⏱️ Time is precious. If you had a magic wand, what would you change?'
    },

    // ─── SECTION 4: Guidance and Recommendations ───
    {
        id: 'discovery_sources',
        type: 'multiple',
        section: 'Section 4: Guidance and Recommendations',
        title: 'Where did you get property recommendations from?',
        hint: 'Select all that apply',
        options: ['Real Estate Agents', 'Property Portals', 'Friends and Family', 'Builders', 'Social Media', 'YouTube', 'Other'],
        transition: () => '🤝 Great insights! Now let\'s talk about the advice and recommendations you received...'
    },
    {
        id: 'recommendation_match',
        type: 'rating',
        section: 'Section 4: Guidance and Recommendations',
        title: 'How well did recommendations match your actual needs?',
        labelLow: 'Never matched',
        labelHigh: 'Always matched',
        transition: (answers) => {
            if (answers.discovery_sources && answers.discovery_sources.includes('Real Estate Agents')) return '🏢 Agents were in the mix! How well did their recommendations actually match your needs?';
            if (answers.discovery_sources && answers.discovery_sources.includes('Friends and Family')) return '👨‍👩‍👧 Family knows best? Let\'s see — how well did recommendations match your needs?';
            return '📡 Interesting sources! How well did recommendations match what you actually wanted?';
        }
    },
    {
        id: 'irrelevant_frequency',
        type: 'rating',
        section: 'Section 4: Guidance and Recommendations',
        title: 'How often did you receive irrelevant property suggestions?',
        labelLow: 'Never irrelevant',
        labelHigh: 'Always irrelevant',
        transition: (answers) => {
            if (answers.recommendation_match <= 4) return '😕 Low match rate — frustrating. How often were suggestions completely irrelevant?';
            if (answers.recommendation_match >= 7) return '👍 Decent matching! But how often did you still get irrelevant suggestions?';
            return '📊 Mixed results. How often did you receive outright irrelevant recommendations?';
        }
    },
    {
        id: 'trust_level',
        type: 'rating',
        section: 'Section 4: Guidance and Recommendations',
        title: 'How much do you trust the recommendations you received?',
        labelLow: 'No trust',
        labelHigh: 'Complete trust',
        transition: (answers) => {
            if (answers.irrelevant_frequency >= 7) return '😤 Lots of irrelevant noise — annoying! How much did you trust what you were told?';
            return '📝 Noted! Overall, how much trust did you place in the recommendations?';
        }
    },
    {
        id: 'verification_frequency',
        type: 'single',
        section: 'Section 4: Guidance and Recommendations',
        title: 'How often did agents push properties that didn\'t match your criteria?',
        options: ['Never', 'Rarely', 'Sometimes', 'Often', 'Always'],
        transition: (answers) => {
            if (answers.trust_level <= 4) return '🤨 Low trust — that says a lot. How often did agents push mismatched properties?';
            return '🔍 Interesting trust level. How often did agents push properties outside your criteria?';
        }
    },
    {
        id: 'mismatch_story',
        type: 'text',
        section: 'Section 4: Guidance and Recommendations',
        title: 'Can you describe a situation where a recommendation didn\'t match reality?',
        placeholder: 'Describe a situation where a recommendation didn\'t match reality...',
        transition: (answers) => {
            if (answers.verification_frequency === 'Always' || answers.verification_frequency === 'Often') return '🕵️ That\'s a lot of mismatches! Can you share a time when a recommendation didn\'t match reality?';
            return '📝 Got it. Can you describe a specific mismatch between recommendation and reality?';
        }
    },
    {
        id: 'trust_drivers',
        type: 'text',
        section: 'Section 4: Guidance and Recommendations',
        title: 'What makes you trust a property recommendation?',
        placeholder: 'What makes you trust a property recommendation?',
        transition: () => '🤔 Interesting story! Now flip it — what WOULD make you trust a recommendation?'
    },

    // ─── SECTION 5: Challenges and Frictions ───
    {
        id: 'budget_mismatch_pain',
        type: 'rating',
        section: 'Section 5: Challenges and Frictions',
        title: 'How much of a problem is budget mismatch in property recommendations?',
        labelLow: 'Not a problem',
        labelHigh: 'Major problem',
        transition: () => '⚡ Almost there! Let\'s zero in on the biggest pain points...'
    },
    {
        id: 'location_mismatch_pain',
        type: 'rating',
        section: 'Section 5: Challenges and Frictions',
        title: 'How much of a problem is location mismatch?',
        labelLow: 'Not a problem',
        labelHigh: 'Major problem',
        transition: (answers) => {
            if (answers.budget_mismatch_pain >= 7) return '💸 Budget mismatch is a big pain! What about location mismatches?';
            return '📝 Noted on budget. How about location — did properties match your area preferences?';
        }
    },
    {
        id: 'description_vs_reality',
        type: 'rating',
        section: 'Section 5: Challenges and Frictions',
        title: 'How much of a problem is the gap between property description and reality?',
        labelLow: 'Not a problem',
        labelHigh: 'Major problem',
        transition: (answers) => {
            if (answers.location_mismatch_pain >= 7) return '📍 Location mismatch too — double frustration! What about descriptions vs reality?';
            return '📍 Got it! How about property descriptions — did they match what you actually saw?';
        }
    },
    {
        id: 'pressure_to_decide',
        type: 'rating',
        section: 'Section 5: Challenges and Frictions',
        title: 'How much pressure do you feel to make quick decisions?',
        labelLow: 'No pressure',
        labelHigh: 'Extreme pressure',
        transition: (answers) => {
            if (answers.description_vs_reality >= 7) return '📸 Misleading descriptions — that breaks trust! Did you feel pressure to decide quickly?';
            return '📝 Description gaps noted. Did you feel pressure to make quick decisions?';
        }
    },
    {
        id: 'hidden_details_pain',
        type: 'rating',
        section: 'Section 5: Challenges and Frictions',
        title: 'How much of a problem are hidden details (fees, legal issues, maintenance)?',
        labelLow: 'Not a problem',
        labelHigh: 'Major problem',
        transition: (answers) => {
            if (answers.pressure_to_decide >= 7) return '😰 High pressure tactics — nobody likes that! Were important details hidden from you?';
            return '📋 Pressure level noted. How about transparency — were details hidden until later?';
        }
    },
    {
        id: 'pain_point_ranking',
        type: 'rank',
        section: 'Section 5: Challenges and Frictions',
        title: 'Rank these pain points from most frustrating (1) to least frustrating (8):',
        items: ['Irrelevant Recommendations', 'Time Wasted Visiting', 'Budget Mismatches', 'Hidden Information', 'Pressure From Agents', 'Too Many Choices', 'Too Few Choices', 'Lack of Trust'],
        transition: (answers) => {
            if (answers.hidden_details_pain >= 7) return '🙈 Hidden information is a big red flag! Now rank all these frustrations...';
            return '📝 Got it! Now let\'s rank all these pain points from worst to least...';
        }
    },
    {
        id: 'top_pain_to_eliminate',
        type: 'single',
        section: 'Section 5: Challenges and Frictions',
        title: 'If you could eliminate ONE problem from your search, which would it be?',
        options: ['Irrelevant Recommendations', 'Time Wasted Visiting', 'Budget Mismatches', 'Hidden Information', 'Pressure From Agents', 'Too Many Choices', 'Too Few Choices', 'Lack of Trust'],
        transition: () => '📊 Great ranking! If you could only fix ONE of these, which would it be?'
    },
    {
        id: 'most_frustrating_moment',
        type: 'text',
        section: 'Section 5: Challenges and Frictions',
        title: 'Describe the single most frustrating moment in your property search.',
        placeholder: 'Describe the most frustrating moment...',
        transition: () => '🎯 Clear priority! Now tell us your single most frustrating moment...'
    },

    // ─── SECTION 6: Exploring a Different Approach ───
    {
        id: 'value_filtered_recs',
        type: 'rating',
        section: 'Section 6: Exploring a Different Approach',
        title: 'How valuable would it be to receive only filtered, highly relevant property recommendations?',
        labelLow: 'Not valuable',
        labelHigh: 'Extremely valuable',
        transition: () => '✨ Final section! We\'re exploring something different — your honest thoughts would mean a lot...'
    },
    {
        id: 'value_fewer_visits',
        type: 'rating',
        section: 'Section 6: Exploring a Different Approach',
        title: 'How valuable would it be if the number of property visits could be reduced by 50%?',
        labelLow: 'Not valuable',
        labelHigh: 'Extremely valuable',
        transition: (answers) => {
            if (answers.value_filtered_recs >= 8) return '🌟 High value — that\'s encouraging! What if we could cut property visits in half?';
            if (answers.value_filtered_recs >= 5) return '🤔 Interesting! What if unnecessary visits could be reduced by 50%?';
            return '📝 Noted. Here\'s another angle — what if property visits were cut in half?';
        }
    },
    {
        id: 'benefit_ranking',
        type: 'rank',
        section: 'Section 6: Exploring a Different Approach',
        title: 'Rank what matters most to you (1 = most important):',
        items: ['Fewer Property Visits', 'Better Property Matches', 'More Trustworthy Recommendations', 'Less Time Spent Searching', 'Greater Transparency', 'Reduced Decision Fatigue'],
        transition: (answers) => {
            if (answers.value_fewer_visits >= 8) return '🎯 Huge value in saving time! Let\'s understand what matters most to you...';
            return '📝 Got it! Now rank what would matter most in an ideal service...';
        }
    },
    {
        id: 'likelihood_to_try',
        type: 'rating',
        section: 'Section 6: Exploring a Different Approach',
        title: 'How likely are you to try a new service that provides curated, verified property recommendations?',
        labelLow: 'Very unlikely',
        labelHigh: 'Very likely',
        transition: () => '📋 Priorities clear! One key question — would you actually try such a platform?'
    },
    {
        id: 'concerns_about_service',
        type: 'text',
        section: 'Section 6: Exploring a Different Approach',
        title: 'What concerns would you have about trying such a service?',
        placeholder: 'What concerns would you have?',
        transition: (answers) => {
            if (answers.likelihood_to_try >= 8) return '🎉 Amazing! What concerns might still hold you back?';
            if (answers.likelihood_to_try >= 5) return '🤔 Open to it! What would you need to feel comfortable trying it?';
            return '👍 Fair enough! What would concern you about trying something new?';
        }
    },
    {
        id: 'interested_in_beta',
        type: 'single',
        section: 'Section 6: Exploring a Different Approach',
        title: 'Would you be interested in being an early user of such a service?',
        options: ['Yes', 'Maybe', 'No'],
        transition: () => '🙏 Thanks for being honest! Last few questions — would you want early access?'
    },
    {
        id: 'contact_details',
        type: 'contact',
        section: 'Section 6: Exploring a Different Approach',
        title: 'Great! Please share your contact details so we can reach out:',
        skipIf: (answers) => answers.interested_in_beta === 'No',
        transition: (answers) => {
            if (answers.interested_in_beta === 'Yes') return '🎊 Fantastic! We\'d love to keep in touch — drop your details below:';
            return '😊 Great — just in case you change your mind, here\'s where to leave your info:';
        }
    },
    {
        id: 'open_to_followup',
        type: 'single',
        section: 'Section 6: Exploring a Different Approach',
        title: 'Would you be open to a 15-minute follow-up conversation?',
        options: ['Yes', 'No'],
        transition: () => '📞 One more — would you be open to a quick follow-up chat?'
    },
    {
        id: 'anything_else',
        type: 'text',
        section: 'Section 6: Exploring a Different Approach',
        title: 'Is there anything else you\'d like to share about your property search experience?',
        placeholder: 'Anything else you\'d like to share?',
        transition: () => '🏁 Last question! Anything else on your mind about property search?'
    },

    // ─── THANK YOU ───
    { id: 'thankyou', type: 'thankyou', section: null }
];

// ─── DERIVED CONSTANTS ───
const totalQuestions = questions.filter(q => q.type !== 'welcome' && q.type !== 'thankyou').length;

// ─── HELPER: Get only survey questions (no welcome/thankyou) ───
function getSurveyQuestions() {
    return questions.filter(q => q.type !== 'welcome' && q.type !== 'thankyou');
}

// ─── PROGRESS BAR ───
function getEffectiveQuestionNumber() {
    let count = 0;
    for (let i = 1; i <= currentQuestion; i++) {
        const q = questions[i];
        if (q && q.type !== 'welcome' && q.type !== 'thankyou') {
            if (!q.skipIf || !q.skipIf(answers)) count++;
        }
    }
    return count;
}

function getEffectiveTotalQuestions() {
    let count = 0;
    for (let i = 0; i < questions.length; i++) {
        const q = questions[i];
        if (q.type !== 'welcome' && q.type !== 'thankyou') {
            if (!q.skipIf || !q.skipIf(answers)) count++;
        }
    }
    return count;
}

function updateProgress() {
    const container = document.getElementById('progressContainer');
    if (currentQuestion === 0 || currentQuestion >= questions.length - 1) {
        container.classList.add('hidden');
        return;
    }
    container.classList.remove('hidden');
    const effective = getEffectiveQuestionNumber();
    const total = getEffectiveTotalQuestions();
    const percent = Math.round((effective / total) * 100);
    document.getElementById('progressText').textContent = `Question ${effective} of ${total}`;
    document.getElementById('progressPercent').textContent = `${percent}%`;
    document.getElementById('progressBar').style.width = `${percent}%`;
}

// ─── MAIN RENDER ───
function renderQuestion() {
    const q = questions[currentQuestion];

    // Handle skip logic
    if (q && q.skipIf && q.skipIf(answers)) {
        currentQuestion++;
        renderQuestion();
        return;
    }

    const content = document.getElementById('surveyContent');
    content.innerHTML = '';
    updateProgress();

    if (q.type === 'welcome') {
        renderWelcome(content);
        return;
    }
    if (q.type === 'thankyou') {
        renderThankYou(content);
        return;
    }

    let html = '';

    // Transition message from the question's own transition function
    if (q.transition) {
        const msg = typeof q.transition === 'function' ? q.transition(answers) : q.transition;
        if (msg) {
            html += `<div class="transition-message">${msg}</div>`;
        }
    }

    html += '<div class="question-card">';
    if (q.section) {
        html += `<div class="section-header">${q.section}</div>`;
    }
    // Render title — wrap bracketed guidance in a styled span
    const titleHtml = q.title.replace(/\n\n\(([^)]+)\)/g, '<br><br><span class="question-guidance">($1)</span>');
    html += `<div class="question-title">${titleHtml}</div>`;
    if (q.hint) {
        html += `<div class="question-hint">${q.hint}</div>`;
    }

    switch (q.type) {
        case 'single': html += renderSingle(q); break;
        case 'multiple': html += renderMultiple(q); break;
        case 'text': html += renderText(q); break;
        case 'numeric': html += renderNumeric(q); break;
        case 'rating': html += renderRating(q); break;
        case 'rank': html += renderRank(q); break;
        case 'contact': html += renderContact(q); break;
    }

    html += renderButtons();
    html += '</div>';

    content.innerHTML = html;
    attachEventListeners(q);
    updateNextButton();
}

// ─── WELCOME & THANK YOU ───
function renderWelcome(container) {
    container.innerHTML = `
        <div class="question-card">
            <div class="welcome-content">
                <h1>🏡 Real Estate Property Search Survey</h1>
                <p>Thank you for taking the time to share your property search experience with us.</p>
                <p>This survey will help us understand the challenges people face when searching for properties in India, and explore how we can make the process better.</p>
                <div class="highlight">
                    <strong>⏱️ Time:</strong> ~10-12 minutes<br>
                    <strong>📝 Questions:</strong> ${totalQuestions} questions across 6 sections<br>
                    <strong>🔒 Privacy:</strong> Your responses are confidential and used only for research purposes.
                </div>
                <p style="margin-top:20px; font-size:14px; color:#9ca3af;">There are no right or wrong answers — we just want your honest experience.</p>
                <button class="btn-begin" onclick="startSurvey()">Begin Survey →</button>
            </div>
        </div>`;
}

function renderThankYou(container) {
    playCompletionSound();
    container.innerHTML = `
        <div class="question-card">
            <div class="thank-you">
                <h2>🙏 Thank you so much for your time!</h2>
                <p style="font-size:18px; margin-bottom:12px;">Your insights are incredibly valuable.</p>
                <p>Your responses have been recorded and will help us build a better property search experience for everyone.</p>
                <p style="margin-top:24px; color:#4F46E5; font-weight:500;">Together, we can make property search less stressful! 🏡✨</p>
            </div>
        </div>`;
    showConfetti();
    submitSurvey();
    console.log('Survey Complete! Responses:', JSON.stringify(answers, null, 2));
}

// ─── QUESTION TYPE RENDERERS ───
function renderSingle(q) {
    let html = '<div class="options-grid">';
    q.options.forEach(opt => {
        const selected = answers[q.id] === opt ? 'selected' : '';
        html += `<div class="option-card ${selected}" data-value="${opt}">
            <div class="option-indicator"></div>
            <span>${opt}</span>
        </div>`;
    });
    html += '</div>';
    return html;
}

function renderMultiple(q) {
    let html = '<div class="options-grid">';
    const selectedArr = answers[q.id] || [];
    q.options.forEach(opt => {
        const selected = selectedArr.includes(opt) ? 'selected' : '';
        html += `<div class="option-card ${selected}" data-value="${opt}" data-type="multi">
            <div class="option-indicator checkbox-indicator"></div>
            <span>${opt}</span>
        </div>`;
    });
    html += '</div>';
    return html;
}

function renderText(q) {
    const val = answers[q.id] || '';
    return `<textarea class="text-input" id="textAnswer" placeholder="${q.placeholder || ''}">${val}</textarea>`;
}

function renderNumeric(q) {
    if (answers[q.id] === undefined) answers[q.id] = 0;
    const val = answers[q.id];
    return `<div class="numeric-container">
        <button class="numeric-btn" onclick="adjustNumeric(-1)">−</button>
        <input type="number" class="numeric-input" id="numericAnswer" value="${val}" min="0" placeholder="${q.placeholder || '0'}">
        <button class="numeric-btn" onclick="adjustNumeric(1)">+</button>
    </div>`;
}

function renderRating(q) {
    const val = answers[q.id] || 0;
    let html = '<div class="rating-container">';
    for (let i = 1; i <= 10; i++) {
        const active = i <= val ? 'active' : '';
        html += `<button class="rating-btn ${active}" data-rating="${i}">${i}</button>`;
    }
    html += '</div>';
    html += `<div class="rating-labels"><span>${q.labelLow}</span><span>${q.labelHigh}</span></div>`;
    return html;
}

function renderRank(q) {
    let html = '<div class="rank-container">';
    const existing = answers[q.id] || {};
    q.items.forEach(item => {
        const val = existing[item] || '';
        html += `<div class="rank-item">
            <label>${item}</label>
            <select class="rank-select" data-item="${item}">
                <option value="">--</option>`;
        for (let i = 1; i <= q.items.length; i++) {
            const sel = val == i ? 'selected' : '';
            html += `<option value="${i}" ${sel}>${i}${getOrdinalSuffix(i)}</option>`;
        }
        html += `</select></div>`;
    });
    html += '</div>';
    return html;
}

function renderContact(q) {
    const val = answers[q.id] || {};
    return `<div class="contact-form">
        <div class="form-group">
            <label for="contactName">Name</label>
            <input type="text" class="form-input" id="contactName" placeholder="Your name" value="${val.name || ''}">
        </div>
        <div class="form-group">
            <label for="contactEmail">Email</label>
            <input type="email" class="form-input" id="contactEmail" placeholder="your@email.com" value="${val.email || ''}">
        </div>
        <div class="form-group">
            <label for="contactPhone">Phone</label>
            <input type="tel" class="form-input" id="contactPhone" placeholder="+91 XXXXX XXXXX" value="${val.phone || ''}">
        </div>
    </div>`;
}

function renderButtons() {
    const isFirst = currentQuestion <= 1;
    const lastSurveyIndex = questions.length - 2; // index before thankyou
    const isLast = currentQuestion >= lastSurveyIndex;
    const q = questions[currentQuestion];
    const autoAdvanceTypes = ['single', 'rating'];
    const needsManualNext = !autoAdvanceTypes.includes(q.type) || isLast;
    return `<div class="btn-container">
        ${!isFirst ? '<button class="btn-back" onclick="goBack()">← Back</button>' : '<div></div>'}
        ${needsManualNext ? `<button class="btn-next" id="nextBtn" onclick="goNext()">${isLast ? 'Submit Survey ✓' : 'Next →'}</button>` : ''}
    </div>`;
}

// ─── SOUND EFFECTS (Web Audio API, no external files) ───
const AudioCtx = window.AudioContext || window.webkitAudioContext;
let audioCtx = null;
function getAudioCtx() { if (!audioCtx) audioCtx = new AudioCtx(); return audioCtx; }

function playClickSound() {
    try {
        const ctx = getAudioCtx();
        const bufferSize = ctx.sampleRate * 0.02;
        const buffer = ctx.createBuffer(1, bufferSize, ctx.sampleRate);
        const data = buffer.getChannelData(0);
        for (let i = 0; i < bufferSize; i++) {
            data[i] = (Math.random() * 2 - 1) * Math.exp(-i / (bufferSize * 0.1));
        }
        const source = ctx.createBufferSource();
        source.buffer = buffer;
        const filter = ctx.createBiquadFilter();
        filter.type = 'bandpass';
        filter.frequency.value = 3000;
        filter.Q.value = 2;
        const gain = ctx.createGain();
        gain.gain.setValueAtTime(0.3, ctx.currentTime);
        gain.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + 0.03);
        source.connect(filter);
        filter.connect(gain);
        gain.connect(ctx.destination);
        source.start(ctx.currentTime);
    } catch(e) {}
}

function playCompletionSound() {
    try {
        const ctx = getAudioCtx();
        const notes = [523, 659, 784, 988, 1175, 1568];
        notes.forEach((freq, i) => {
            const osc = ctx.createOscillator();
            const gain = ctx.createGain();
            osc.connect(gain);
            gain.connect(ctx.destination);
            osc.type = 'sine';
            const startTime = ctx.currentTime + i * 0.1;
            osc.frequency.setValueAtTime(freq, startTime);
            gain.gain.setValueAtTime(0.15, startTime);
            gain.gain.setValueAtTime(0.15, startTime + 0.08);
            gain.gain.exponentialRampToValueAtTime(0.001, startTime + 0.5);
            osc.start(startTime);
            osc.stop(startTime + 0.5);
        });
        setTimeout(() => {
            const chord = [1047, 1319, 1568];
            chord.forEach(freq => {
                const osc = ctx.createOscillator();
                const gain = ctx.createGain();
                osc.connect(gain);
                gain.connect(ctx.destination);
                osc.type = 'sine';
                osc.frequency.setValueAtTime(freq, ctx.currentTime);
                gain.gain.setValueAtTime(0.1, ctx.currentTime);
                gain.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + 1.0);
                osc.start(ctx.currentTime);
                osc.stop(ctx.currentTime + 1.0);
            });
        }, 600);
    } catch(e) {}
}

function autoAdvance() {
    playClickSound();
    setTimeout(() => goNext(), 400);
}

// ─── EVENT LISTENERS ───
function attachEventListeners(q) {
    const lastSurveyIndex = questions.length - 2;

    // Single select — auto-advance on click
    if (q.type === 'single') {
        document.querySelectorAll('.option-card').forEach(card => {
            card.addEventListener('click', () => {
                document.querySelectorAll('.option-card').forEach(c => c.classList.remove('selected'));
                card.classList.add('selected');
                answers[q.id] = card.dataset.value;

                // Check exit logic
                if (q.exitIf && q.exitIf(answers)) {
                    // Fill all remaining questions with 'NA'
                    const currentIdx = questions.indexOf(q);
                    for (let i = currentIdx + 1; i < questions.length; i++) {
                        const remaining = questions[i];
                        if (remaining.type !== 'thankyou') {
                            answers[remaining.id] = 'NA';
                        }
                    }
                    playClickSound();
                    setTimeout(() => {
                        // Jump to thankyou
                        currentQuestion = questions.length - 1;
                        renderQuestion();
                    }, 400);
                    return;
                }

                // Auto-advance unless it's the last question
                if (currentQuestion < lastSurveyIndex) {
                    autoAdvance();
                } else {
                    updateNextButton();
                }
            });
        });
    }

    // Multiple select
    if (q.type === 'multiple') {
        document.querySelectorAll('.option-card[data-type="multi"]').forEach(card => {
            card.addEventListener('click', () => {
                card.classList.toggle('selected');
                const selected = [];
                document.querySelectorAll('.option-card.selected').forEach(c => selected.push(c.dataset.value));
                answers[q.id] = selected;
                updateNextButton();
            });
        });
    }

    // Text
    if (q.type === 'text') {
        const textarea = document.getElementById('textAnswer');
        if (textarea) {
            textarea.addEventListener('input', () => {
                answers[q.id] = textarea.value;
                updateNextButton();
            });
        }
    }

    // Numeric
    if (q.type === 'numeric') {
        const input = document.getElementById('numericAnswer');
        if (input) {
            input.addEventListener('input', () => {
                answers[q.id] = parseInt(input.value) || 0;
                updateNextButton();
            });
        }
    }

    // Rating — auto-advance on click
    if (q.type === 'rating') {
        document.querySelectorAll('.rating-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                const val = parseInt(btn.dataset.rating);
                answers[q.id] = val;
                document.querySelectorAll('.rating-btn').forEach((b, i) => {
                    b.classList.toggle('active', (i + 1) <= val);
                });
                if (currentQuestion < lastSurveyIndex) {
                    autoAdvance();
                } else {
                    updateNextButton();
                }
            });
        });
    }

    // Rank
    if (q.type === 'rank') {
        document.querySelectorAll('.rank-select').forEach(sel => {
            sel.addEventListener('change', () => {
                const rankData = {};
                document.querySelectorAll('.rank-select').forEach(s => {
                    if (s.value) rankData[s.dataset.item] = parseInt(s.value);
                });
                answers[q.id] = rankData;
                updateNextButton();
            });
        });
    }

    // Contact
    if (q.type === 'contact') {
        ['contactName', 'contactEmail', 'contactPhone'].forEach(id => {
            const el = document.getElementById(id);
            if (el) {
                el.addEventListener('input', () => {
                    answers[q.id] = {
                        name: document.getElementById('contactName').value,
                        email: document.getElementById('contactEmail').value,
                        phone: document.getElementById('contactPhone').value
                    };
                    updateNextButton();
                });
            }
        });
    }
}

// ─── VALIDATION ───
function updateNextButton() {
    const btn = document.getElementById('nextBtn');
    if (!btn) return;
    const q = questions[currentQuestion];
    let hasAnswer = false;

    switch (q.type) {
        case 'single':
            hasAnswer = !!answers[q.id];
            break;
        case 'multiple':
            hasAnswer = answers[q.id] && answers[q.id].length > 0;
            break;
        case 'text':
            hasAnswer = !!(answers[q.id] && answers[q.id].trim());
            break;
        case 'numeric':
            hasAnswer = answers[q.id] !== undefined;
            break;
        case 'rating':
            hasAnswer = !!answers[q.id];
            break;
        case 'rank':
            hasAnswer = answers[q.id] && Object.keys(answers[q.id]).length === q.items.length;
            break;
        case 'contact':
            const c = answers[q.id];
            const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            const phoneRegex = /^[\d\s\+\-()]{7,15}$/;
            hasAnswer = c && c.name && c.name.trim() && emailRegex.test(c.email) && phoneRegex.test(c.phone);
            break;
    }

    btn.disabled = !hasAnswer;
}

// ─── NAVIGATION ───
function startSurvey() {
    currentQuestion = 1;
    renderQuestion();
}

function goNext() {
    playClickSound();
    const lastSurveyIndex = questions.length - 2;
    if (currentQuestion >= lastSurveyIndex) {
        // Move to thankyou
        currentQuestion = questions.length - 1;
        renderQuestion();
        return;
    }
    currentQuestion++;
    renderQuestion();
    window.scrollTo({ top: 0, behavior: 'smooth' });
}

function goBack() {
    if (currentQuestion <= 1) return;
    currentQuestion--;
    // Skip conditional questions when going back
    const q = questions[currentQuestion];
    if (q && q.skipIf && q.skipIf(answers)) {
        currentQuestion--;
    }
    renderQuestion();
    window.scrollTo({ top: 0, behavior: 'smooth' });
}

function adjustNumeric(delta) {
    const input = document.getElementById('numericAnswer');
    if (!input) return;
    let val = parseInt(input.value) || 0;
    val = Math.max(0, val + delta);
    input.value = val;
    const q = questions[currentQuestion];
    answers[q.id] = val;
    updateNextButton();
}

function getOrdinalSuffix(n) {
    const s = ['th', 'st', 'nd', 'rd'];
    const v = n % 100;
    return s[(v - 20) % 10] || s[v] || s[0];
}

// ─── CONFETTI ───
function showConfetti() {
    const container = document.createElement('div');
    container.className = 'confetti-container';
    document.body.appendChild(container);

    const colors = ['#4F46E5', '#7c3aed', '#ec4899', '#f59e0b', '#10b981', '#06b6d4'];
    for (let i = 0; i < 60; i++) {
        const confetti = document.createElement('div');
        confetti.className = 'confetti';
        confetti.style.left = Math.random() * 100 + '%';
        confetti.style.background = colors[Math.floor(Math.random() * colors.length)];
        confetti.style.animationDelay = Math.random() * 2 + 's';
        confetti.style.animationDuration = (2 + Math.random() * 2) + 's';
        confetti.style.width = (6 + Math.random() * 8) + 'px';
        confetti.style.height = (6 + Math.random() * 8) + 'px';
        confetti.style.borderRadius = Math.random() > 0.5 ? '50%' : '2px';
        container.appendChild(confetti);
    }

    setTimeout(() => container.remove(), 5000);
}

// ─── SURVEY SUBMISSION ───
async function submitSurvey() {
    try {
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = "https://script.google.com/macros/s/AKfycbyh3VcaDAubmc8J3x71G2TfXdQlDaHrMLWx_npE0r8600pSDlmlro2zm5ClyEebK2bkFw/exec";
        form.style.display = 'none';

        // Send ALL answer keys dynamically
        const input = document.createElement('input');
        input.type = 'hidden';
        input.name = 'data';
        input.value = JSON.stringify(answers);
        form.appendChild(input);

        // Hidden iframe to prevent page navigation
        const iframe = document.createElement('iframe');
        iframe.name = 'submitFrame';
        iframe.style.display = 'none';
        document.body.appendChild(iframe);
        form.target = 'submitFrame';

        document.body.appendChild(form);
        form.submit();

        // Cleanup
        setTimeout(() => {
            form.remove();
            iframe.remove();
        }, 3000);

        console.log("Survey submitted successfully");
    } catch(error) {
        console.error("Submission failed", error);
    }
}

// ─── INITIALIZE ───
renderQuestion();
